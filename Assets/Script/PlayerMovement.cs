...
public class PlayerMovement : MonoBehaviour
{
    ...
    private float m_JumpEladpsedTime;
    ...
    private void Move()
    {
        ...
        m_Body.MovePosition(m_Body.position + m_Movement * Time.fixedDeltaTime * speed);
    }
    ...
    private void Jump()
    {
        if(m_IsJumping && m_JumpEladpsedTime < m_JumpTime / 3)
        {
        ...
        if (m_IsJumping && m_JumpEladpsedTime < m_JumpTime)
        {
            m_JumpEladpsedTime += Time.fixedDeltaTime;
            float proportionCompleted = Mathf.Clamp01(m_JumpEladpsedTime / m_JumpTime);
